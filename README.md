...
**b) Vetor após a primeira extração da raiz:** `[30, 18, 3, 6, 12, 45]` *(A lógica aqui é que o 45, sendo o maior elemento, vai ordenado lá para o final do vetor. Aí o 30 assume o topo para podermos rodar o Heapify de novo).*

> **Nota:** Deixei a prova de mesa disso rodando direto no código `avaliacao_heapsort.c` que subi aqui no repositório.

 # Questão 3 **Resposta:** Alternativa correta: 
 **A) Apenas II e IV** * **I é falsa:** O ShellSort não é estável de jeito nenhum, já que ele dá saltos longos e acaba trocando a ordem relativa de elementos iguais.
...
**Justificativa:** Quando a gente fala de milhões de registros, o que mais importa é a **previsibilidade**. O sistema não pode travar do nada. O HeapSort vai garantir um tempo de O(n log n) e um gasto de memória extra de O(1), não importa se os dados chegarem ordenados, de trás pra frente ou uma bagunça total. Usar o ShellSort seria um tiro no pé num cenário desses: se calhar de vir uma base de dados que caia no pior caso dos gaps dele, a performance despenca para O(n²) e o servidor trava.
 ...
 # Questão 5 **Resposta:** 
 A estrutura Heap é perfeita para criar "Filas de Prioridade" (Priority Queues). Um exemplo clássico disso em grafos é o **Algoritmo de Dijkstra** (usado para achar caminhos mais curtos, como rotas de GPS). 
